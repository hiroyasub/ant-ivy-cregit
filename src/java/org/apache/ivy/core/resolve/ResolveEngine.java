...
condition|)
block|{
comment|// create new resolve options because this is a different resolve than the real resolve
comment|// (which will be a resolve of a newCallerInstance module)
...
argument_list|)
expr_stmt|;
comment|// we will continue the resolve anyway to get a nice error message back
comment|// to the user, however reduce the amount of logging in this case
name|optionsToUse
operator|.
name|setLog
argument_list|(
...
argument_list|)
expr_stmt|;
comment|// we store the resolved dependencies revisions and statuses per asked dependency
comment|// revision id,
...
argument_list|(
literal|"getResolvedId() is null for "
operator|+
name|dependencies
index|[
name|i
...
name|NullPointerException
argument_list|(
literal|"getDependencyRevisionId() "
operator|+
literal|"is null for "
operator|+
name|dd
...
name|warn
argument_list|(
literal|"\tmerged artifact not found: "
operator|+
name|adrs
index|[
name|j
...
argument_list|(
literal|"requested configuration"
operator|+
operator|(
name|missingConfs
operator|.
...
comment|// if a module has been evicted then all its dependencies required only by it should be
comment|// evicted too. Since nodes are now sorted from the more dependent to the less one, we
comment|// can traverse the list and check only the direct parent and not all the ancestors
for|for
control|(
name|ListIterator
name|iter
...
condition|)
block|{
comment|/*              * we don't resolve conflicts before loading data for direct dependencies on dynamic              * revisions, so that direct dynamic revisions are always resolved, which is mandatory              * for proper replacement of dynamic revisions during 'deliver'              */
...
condition|)
block|{
comment|// the node is not yet loaded, we can simply update its set of
comment|// configurations to fetch
name|selected
operator|.
name|updateConfsToFetch
argument_list|(
...
block|{
comment|// the node has already been loaded, we must fetch its dependencies in the
comment|// required conf
name|fetchDependencies
argument_list|(
name|node
operator|.
...
name|key
init|=
name|getDependenciesFetchedKey
argument_list|(
name|node
argument_list|,
name|conf
...
if|if
condition|(
name|checkConflictSolvedEvicted
argument_list|(
name|node
argument_list|,
name|ancestor
...
condition|)
block|{
comment|// job is done and node is evicted, nothing to do
return|return
literal|true
return|;
block|}
...
condition|)
block|{
comment|// job is done and node is selected, nothing to do for this ancestor, but we still have
comment|// to check higher levels, for which conflict resolution might have been impossible
comment|// before
if|if
condition|(
name|resolveConflict
...
condition|)
block|{
comment|// now that conflict resolution is ok in ancestors
comment|// we just have to check if the node wasn't previously evicted in root ancestor
name|EvictionData
name|evictionData
init|=
name|node
...
operator|!=
name|ancestor
comment|// we are not handling the direct parent
operator|&&
name|conflictManager
operator|==
name|settings
...
argument_list|()
argument_list|)
comment|// the conflict manager is the default one
operator|&&
name|node
operator|.
name|getParent
...
name|conflicts
argument_list|)
comment|// there is no new conflict in this ancestor
condition|)
block|{
comment|// IVY-465 case
if|if
condition|(
name|settings
operator|.
...
name|debug
argument_list|(
literal|"no new conflicting revisions for "
operator|+
name|node
operator|+
literal|" in "
...
name|RuntimeException
argument_list|(
literal|"problem while parsing repository module descriptor for "
operator|+
name|id
operator|+
literal|": "
