...
comment|// revision id, for direct dependencies only.
...
condition|)
block|{
comment|// if we were forced to this revision and we
comment|// are not a dynamic revision, reset to the
comment|// asked revision
name|depResolvedId
operator|=
name|depRevisionId
expr_stmt|;
...
argument_list|()
decl_stmt|;
comment|// The evicted modules have no description, so we can't put the status
name|String
name|status
init|=
name|depDescriptor
...
name|verbose
argument_list|(
literal|"\tresolved ivy file produced in cache"
argument_list|)
expr_stmt|;
name|report
operator|.
