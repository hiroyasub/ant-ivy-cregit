...
argument_list|()
decl_stmt|;
comment|// true if the depedency descriptor of any of the depender *doesn't* have an explicit
comment|// "<artifact>" or an "<include>". false otherwise
name|boolean
name|atLeastOneDependerNeedsAllArtifacts
init|=
literal|false
decl_stmt|;
comment|// true if the dependency descriptor of any of the depender either has an explicit "<artifact>"
comment|// or an "<include>". false otherwise
name|boolean
name|atLeastOneDependerHasSpecificArtifactSelection
init|=
literal|false
decl_stmt|;
for|for
...
block|}
block|}
comment|// so there's at least one depender D1 which has a specific artifact dependency and at the
comment|// same time there's a depender D2 which doesn't have any explicit artifact/includes.
comment|// so it is expected that an implicit "include all artifacts" is applied so that dependencies
comment|// such as D2 get (all) the artifacts that are published by the dependency's module
if|if
condition|(
name|atLeastOneDependerHasSpecificArtifactSelection
operator|&&
...
condition|)
block|{
comment|// add a "include all artifacts" rule
name|dependencyIncludes
operator|.
name|add
argument_list|(
name|includeAllArtifacts
argument_list|()
argument_list|)
expr_stmt|;
block|}
