...
return|;
block|}
comment|/**      * True if this cache should check lastmodified date to know if ivy files are up to date.      *       * @return      */
specifier|public
name|boolean
...
condition|)
block|{
comment|// not declared as compressed, nothing to do
return|return;
block|}
comment|// the artifact for the folder of the uncompressed data
name|Artifact
name|uncompressed
init|=
name|adr
operator|.
name|buildUncompressedArtifact
argument_list|()
decl_stmt|;
name|File
name|archiveFile
...
name|setDownloadDetails
argument_list|(
literal|"The compressed artifact "
operator|+
name|artifact
operator|.
name|getId
argument_list|()
operator|+
literal|" could not be uncompressed ("
operator|+
name|e
operator|.
name|getMessage
...
condition|)
block|{
comment|// but if the local archive doesn't exist, trust the cache only if the cached origin
comment|// says that the remote resource doesn't exist either
return|return
operator|!
name|savedOrigin
operator|.
