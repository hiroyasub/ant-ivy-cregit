...
block|}
block|}
comment|// This is a cludge to reconcile different values passed to the method
specifier|public
specifier|static
name|String
name|substituteTokens
...
argument_list|()
decl_stmt|;
name|checkAgainstPathTraversal
argument_list|(
name|pattern
argument_list|,
name|afterTokenSubstitution
...
return|;
block|}
comment|/**      * This class returns a captured value after validating it doesn't      * contain any path traversal sequence.      *      *<p>{@code toString}</p> will be invoked when the value is      * actually used as a token inside of a pattern passed to {@link      * #substituteTokens}.</p>      */
specifier|private
specifier|static
class|class
name|Validated
...
name|tokenValue
operator|+
literal|" contains an illegal path sequence"
argument_list|)
throw|;
block|}
block|}
...
argument_list|()
decl_stmt|;
comment|// it is OK to have a token root containing .. sequences
if|if
condition|(
name|root
operator|.
...
block|}
name|String
name|patternedPartWithNormalizedSlashes
init|=
name|afterTokenSubstitution
operator|.
name|substring
...
name|IllegalArgumentException
argument_list|(
literal|"path after token expansion contains an illegal sequence"
argument_list|)
throw|;
block|}
block|}
