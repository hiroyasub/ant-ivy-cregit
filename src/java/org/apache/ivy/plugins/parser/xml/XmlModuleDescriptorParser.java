...
expr_stmt|;
block|}
comment|// Resolve parent module descriptor from module inheritance repository
name|parent
operator|=
...
name|debug
argument_list|(
literal|"Registering parent module into module inheritance repository map. Parent module location: "
operator|+
name|urlString
argument_list|)
expr_stmt|;
...
argument_list|)
expr_stmt|;
comment|// Do we even need to be adding this resolver to the Ivy settings considering that it's being placed in the map and not being used elsewhere?
name|ivyContext
operator|.
name|getSettings
argument_list|()
...
argument_list|)
decl_stmt|;
comment|// The parent resolver will be null if its dev-only filesystem path hasn't been specified via the location attribute of the extends element.
if|if
condition|(
name|resolver
operator|==
