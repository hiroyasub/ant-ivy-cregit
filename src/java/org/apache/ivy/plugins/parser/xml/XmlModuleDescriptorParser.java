...
name|ivySettings
expr_stmt|;
name|configureModuleInheritanceRepository
argument_list|()
expr_stmt|;
block|}
comment|/**          * Configure module inheritance repository (repository containning all path references           * to parent modules). Basicaly it checks if module inheritance repository exist           * in current ivy context, otherwise it will create one as a {@link URLResolver}          */
specifier|protected
name|void
name|configureModuleInheritanceRepository
parameter_list|()
...
literal|null
decl_stmt|;
comment|//Does module-inheritance-repository exists ?
for|for
control|(
name|Iterator
name|iterator
...
block|}
block|}
comment|//if does not exist create one
if|if
condition|(
name|parentModuleResolver
operator|==
...
expr_stmt|;
block|}
comment|// resolve parent from module inheritance repository
name|parent
operator|=
...
block|}
block|}
comment|/**          * Check if parent module is reachable using location attribute (for dev purpose).          * If parent module is reachable it will be registered in module inheritance repository           * @param location a given location           * @throws IOException          * @throws ParseException           */
specifier|protected
name|void
name|checkParentModuleOnFilesystem
parameter_list|(
...
argument_list|)
decl_stmt|;
comment|//is parent module reachable using location attribute ?
if|if
condition|(
name|url
operator|.
...
name|ParseException
argument_list|(
literal|"Unable to find module inheritance repository"
argument_list|,
literal|0
argument_list|)
throw|;
...
name|debug
argument_list|(
literal|"Registering parent module into module inheritance repository, parent module location is "
operator|+
name|url
operator|.
name|toExternalForm
...
return|;
block|}
comment|/**          * Resolve parent module from module inhertance repository          * @param parentMrid a given {@link ModuleRevisionId} to find          * @return a {@link ModuleDescriptor} if found. Return null if no {@link ModuleDescriptor} was found          * @throws ParseException          */
specifier|protected
name|ModuleDescriptor
name|resolveParentFromModuleInheritanceRepository
parameter_list|(
...
name|debug
argument_list|(
literal|"Trying to load included ivy file from module inheritance repository "
argument_list|)
expr_stmt|;
name|DependencyDescriptor
name|dd
