...
return|;
block|}
comment|/**          * We return null where certain elements within a pom don't have a value specified.          * For example, there are pom.xml out there which just use "<classifier/>" in the dependencies.          * (dependencies in org.seleniumhq.selenium:selenium-java:3.141.59 are one such example)          * We do this so that callers of such elements don't have to keep repeating checks for empty value.          * For us an empty value, for many of such elements, is really the same as that element not being specified          *          * @param val The value to check          * @return          */
specifier|private
name|String
name|emptyIsNull
parameter_list|(
