...
argument_list|()
decl_stmt|;
comment|// add profile specific properties
for|for
control|(
specifier|final
name|PomProfileElement
...
continue|continue;
block|}
comment|// keep track of this active profile for later use
name|activeProfiles
operator|.
name|add
argument_list|(
...
condition|)
block|{
comment|// In Maven, "import" scope semantics are equivalent to getting (only) the
comment|// dependency management section of the imported module, into the current
comment|// module, so that those "managed dependency versions" are usable/applicable
comment|// in the current module's dependencies
name|ModuleRevisionId
name|importModRevID
init|=
name|ModuleRevisionId
...
condition|)
block|{
comment|// IVY-1588: we "reset" the parent tracking, since the parent tracking should only be
comment|// non-null when we are parsing a parent pom.
name|previousParents
operator|=
name|IvyContext
operator|.
...
condition|)
block|{
comment|// switch back to the previous state of the parent tracking
name|IvyContext
operator|.
name|getContext
argument_list|()
