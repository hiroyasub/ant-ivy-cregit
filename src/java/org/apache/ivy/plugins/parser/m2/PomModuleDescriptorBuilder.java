...
block|{
comment|/**      * The namespace URI which is used to refer to Maven (pom) specific elements within a      * Ivy module descriptor file (ivy.xml)      */
specifier|private
specifier|static
specifier|final
name|String
name|IVY_XML_MAVEN_NAMESPACE_URI
init|=
literal|"http://ant.apache.org/ivy/maven"
decl_stmt|;
specifier|private
specifier|static
specifier|final
...
argument_list|)
decl_stmt|;
comment|// the same dependency mrid could appear twice in the module descriptor,
comment|// so we check if we already have created a dependency descriptor for the dependency mrid
specifier|final
name|DependencyDescriptor
name|existing
init|=
...
block|}
block|}
comment|// intentional identity check to make sure we don't re-add the same dependency
if|if
condition|(
name|existing
operator|!=
...
argument_list|()
decl_stmt|;
comment|// dependency descriptor keyed by its dependency revision id
specifier|private
specifier|final
name|Map
argument_list|<
