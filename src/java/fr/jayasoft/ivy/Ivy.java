...
expr_stmt|;
block|}
comment|/**      * Call this method to ask ivy to configure some variables using a remote properties file      */
specifier|public
name|void
...
block|}
block|}
comment|// produce resolved ivy conf file in cache
name|File
name|ivyFileInCache
init|=
...
parameter_list|)
block|{
comment|// Note that classical Comparator do not work here, because
comment|// one to one comparison is not suffisant since we only use
comment|// direct dependencies and not transitive one
name|List
name|sorted
init|=
operator|new
...
argument_list|()
decl_stmt|;
comment|// we iterate over the original list reversely, so that last traversed are the first in order
comment|// since we place md by default at the beginning of the result, this preserves the order
comment|// if it doesn't need to be changed (no dependency relationship between mds).
name|List
name|from
init|=
operator|new
...
argument_list|)
decl_stmt|;
comment|// copy to be able to traverse reversely
for|for
control|(
name|int
name|i
...
argument_list|)
decl_stmt|;
comment|// a list of sorted mds that should be after the current md
name|List
name|after
init|=
operator|new
...
argument_list|()
decl_stmt|;
comment|// a list of sorted mds that should be after current md and which are not
name|List
name|between
init|=
operator|new
...
argument_list|()
decl_stmt|;
comment|// find its place in current sorted list
name|int
name|place
init|=
literal|0
decl_stmt|;
comment|// let's check dependency relation ship with others
for|for
control|(
name|ListIterator
name|it2
...
condition|)
block|{
comment|// it depends on another, it has to be placed after
name|place
operator|=
name|it2
operator|.
