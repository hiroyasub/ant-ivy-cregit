...
literal|null
decl_stmt|;
comment|// Collection(IvyNode): direct path from root to this node. Note that the colleciton is ordered but is not a list implementation
specifier|private
name|String
name|_parentConf
init|=
...
argument_list|()
decl_stmt|;
comment|// Map (ModuleIdConf -> Set(ModuleRevisionId)) // map indicating for each dependency which revision has been selected
specifier|private
name|Map
name|_evictedDeps
init|=
...
literal|false
decl_stmt|;
comment|// this map contains all the module ids calling this one as keys
comment|// the mapped nodes correspond to a direct caller from which the transitive caller comes
specifier|private
name|Map
name|_allCallers
init|=
...
return|;
block|}
comment|//        IvyNode root = this;
comment|//        Collection path = new HashSet();
comment|//        path.add(root);
comment|//        while (root.getParent() != null&& !root.isRoot()) {
comment|//            if (path.contains(root.getParent())) {
comment|//                return root;
comment|//            }
comment|//            root = root.getParent();
comment|//            path.add(root);
comment|//        }
comment|//        return root;
block|}
